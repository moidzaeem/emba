    ...
    <form action="{{ route('courses.update', $course->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            >
                ...
                <option value="Whole" {{ old('course_type', $course->course_type) == 'Whole' ? 'selected' : '' }}>Whole</option>
                <option value="Healthcare" {{ old('course_type', $course->course_type) == 'Healthcare' ? 'selected' : '' }}>Healthcare</option>
                <option value="Finance" {{ old('course_type', $course->course_type) == 'Finance' ? 'selected' : '' }}>Finance</option>
            </select>
            @error('course_type')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        ...
        <div class="mb-3">
            ...
            <input
                ...
                id="group_size"
                name="group_size"
                value="{{ old('group_size', $course->group_size) }}"
                min="2"
                max="6"
                required
            >
